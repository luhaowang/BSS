...
randomly assign battery to a target level
...
def batteryCharAssigner(ResBatArray,ArriRate,deltaT,ENG):
    ...
    print N
    mxij = len(ResBatArray)*len(SOC_LEVEL)
    mj = len(SOC_LEVEL)
    mi = len(ResBatArray)
    x = eng.zeros(mxij,1)
    changelist = []
    ...
    BatChosIndex = []
    for i in range(mi):
        BatChosIndex.append(i)
        staylist.append(i)
    for i in range(len(N)):
        batCandiidx = []
        lidx = len(N)-i-1
        num = N[lidx]
        for j in range(len(BatChosIndex)):
            if (ResBatArray[BatChosIndex[j]].vmax * deltaT >= (SOC_LEVEL[lidx]*SOC_NOM - ResBatArray[BatChosIndex[j]].soc ) ):
                batCandiidx.append(BatChosIndex[j])
        for k in range(num): # choose num 
            if len(batCandiidx) == 0:
                return []
            random_index = randrange(0,len(batCandiidx))
            bat_idx = batCandiidx[random_index]
            changelist.append(bat_idx)
            BatChosIndex.remove(bat_idx)
            x[bat_idx *mj + lidx][0] = 1.0
            batCandiidx.pop(random_index) 
    for j in range(len(changelist)):
        staylist.remove(changelist[j])
    for j in range(len(staylist)):
        orilev = int(floor(ResBatArray[staylist[j]].soc / 25.0))
        x[staylist[j] *mj + orilev][0] = 1.0
    return x
...
def chargingScheduler(battery_array,time):
    print "Average chargingScheduler is calculating the optimal Charging rate at time %f. \n" %(time)
    tpoints = []
    tpoints.append(time)
    
    for i in range(len(battery_array)):
            battery_array[i].set_rate((battery_array[i].soc_target - battery_array[i].soc )/(battery_array[i].td - battery_array[i].ts))
...
if __name__ == '__main__':
    ...
    while(EventQ.hasNext()):
        ...
        if ETYPE[eventNow.type] == "EPOCH":
            ...
            if NeedCharge==True: 
                chargingScheduler(battery_array,eventNow.time)  
            #printEventQ(EventQ)
    ...
    data_save = open("cumudata__ran_avg_online_%d_%d_%d.dat" %(BatTotNum,Traffic,Failcount ),'w')
